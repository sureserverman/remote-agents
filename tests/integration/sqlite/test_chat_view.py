...
def test_a_chat_with_no_live_view_yet_has_no_anchor(tmp_path) -> None:
    store = SQLiteChatViewStore(open_ui_database(tmp_path / "ui.sqlite3"))
...
def test_an_anchor_survives_the_connection_that_recorded_it(tmp_path) -> None:
    ...
    assert SQLiteChatViewStore(open_ui_database(database)).anchor(_CHAT) == 100
...
def test_a_sequence_of_writers_leaves_the_first_anchor_standing(tmp_path) -> None:
    ...
    """
    database = tmp_path / "ui.sqlite3"
    writers = [SQLiteChatViewStore(open_ui_database(database)) for _ in range(20)]
...
def test_concurrent_writers_cannot_both_adopt_the_same_chat_s_first_anchor(tmp_path) -> None:
    ...
    open_ui_database(database).close()
    writers = 8
    ...
    def adopt(index: int) -> None:
        store = SQLiteChatViewStore(open_ui_database(database))
    ...
    anchor = SQLiteChatViewStore(open_ui_database(database)).anchor(_CHAT)
