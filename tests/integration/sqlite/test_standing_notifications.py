...
def test_a_session_with_no_notification_yet_has_none(tmp_path) -> None:
    store = SQLiteStandingNotificationStore(open_ui_database(tmp_path / "ui.sqlite3"))
...
async def test_retiring_a_notification_leaves_the_feed_its_observation(tmp_path) -> None:
    ...
    """
    # Two stores now, because the split put them in two files — which is the seam this test
    # exists for, drawn more sharply than when one connection served both.
    from remote_agents.adapters.sqlite.database import open_database

    domain = open_database(tmp_path / "sessions.sqlite3")
    feed = SQLiteActivityStore(domain)
    standing = SQLiteStandingNotificationStore(open_ui_database(tmp_path / "ui.sqlite3"))
...
def test_the_class_of_ask_survives_the_round_trip(tmp_path) -> None:
    ...
    connection = open_ui_database(database)
