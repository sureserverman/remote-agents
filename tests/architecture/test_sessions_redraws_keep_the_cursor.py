...
def test_the_dashboards_sessions_pane_states_its_vanished_row_answer() -> None:
    ...
    **Both parameters, not just the interesting one.** `was_populated` defaults to `True`,
    ...
    # Imported by name, so the call is an `ast.Name`, not an `ast.Attribute`. Both spellings
    # are swept so moving to `base.restore_highlight_by_id` cannot silently drop the check.
    calls = _calls_named(tree, "restore_highlight_by_id") + [
        node
        for node in ast.walk(tree)
        if isinstance(node, ast.Call)
    ...
    silent = [
        ...
        (_enclosing_function(tree, call), sorted(missing))
        for call in calls
