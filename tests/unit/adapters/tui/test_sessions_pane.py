...
async def test_the_cursor_rests_painted_on_a_row_whose_enter_does_not_mutate() -> None:
    ...
    **Asserted on the cursor's band rather than on its whole style**, and the change is what the
    cursor/target split made necessary. The component style is a background, a foreground and
    `bold`; a row's own content colours override the foreground, and they always did — the state
    word has been `$success` since the redesign. What changed is that the *identity* is coloured
    too on the row that is the acting target, so this row no longer has a single non-blank
    segment wearing the component style unmodified, and an equality check that used to pass by
    the identity happening to be plain would now report an undrawn cursor on a cursor that is
    plainly drawn. The band is what the cursor *is* — `ChoiceScreen`'s own CSS says so in one
    line, `background: $selection` — so that is what this asks about.
    """
    shown: list[str] = []
    ...
    async with app.run_test(size=(120, 30)) as pilot:
        ...
        resting = [
            segment
            for segment in choices.render_line(0)
            if segment.text.strip() and segment.style is not None
        ]
        assert resting, "the resting row drew no text at all"
        assert all(segment.style.bgcolor == cursor.bgcolor for segment in resting), (
            "the sessions pane drew no cursor band on its resting row"
        )
        # The row's own signals survive the band, which is what the band being a *background*
        # buys and what DEC-010 needs: the state word stays its group's colour under the cursor
        # exactly as it is off it.
        assert {segment.style.color for segment in resting} != {cursor.color}, (
            "the cursor flattened the row's own colours, so the state word says nothing under it"
        )
