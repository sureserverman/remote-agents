...
uv tool install --managed-python \
  "remote-agents @ git+https://github.com/sureserverman/remote-agents@v0.44.0"
...
```bash
curl -fsSL https://raw.githubusercontent.com/sureserverman/remote-agents/main/scripts/install.sh \
  | REMOTE_AGENTS_VERSION=v0.44.0 bash
remote-agents onboard --install-daemon
```
...
**A session owns one message, and later news replaces it.** The first observation sends that
message; each later report sends a new one carrying the story so far and deletes the one it
supersedes. Three turns half an hour apart used to leave three messages in the chat, each
pushing the menu further up; they now leave one, and it is the newest thing in the chat rather
than buried where the first report landed. The `Open session` button moves onto the
replacement, so it keeps working across the swap.
...
scrolling past the notifications. A replacement counts here exactly like a first notification —
it is a new message and it lands below the menu — so any pass that delivers moves the menu. The
move re-binds the
...
limits are read every 300 seconds**, one call per tick to the same
argumentless `Backend.limits` both surfaces already use, which answers for every provider at
once. Unlike `activity_poll_seconds` that period is not
...
4. With the agent in front, confirm the sessions pane lists it, and that the session you just
   launched is the row the cursor is on and the row marked `▸` in yellow — starting a session
   ...
   row keys `a i r s c f m`, and its **hint row** beneath the status names `d` and `p`. On a
   ...
   action.
5. **The cursor drill, which is what makes `s` safe to have at all.** Two halves:

   a. With two or more sessions listed, press the down arrow and confirm the yellow `▸` moves
      with the cursor, every press. The marker is the cursor drawn so it reads from another
      pane, not a second selection — a marker that lagged behind would say one session is about
      to be stopped while `s` stops a different one.

   b. Put the cursor on the second session and end that session from Telegram. Within ten
      seconds the list re-reads itself; confirm the cursor is left on **no row at all** rather
      than jumping to the first one, that **no row is marked**, and that pressing `s` at that
      moment does nothing. Press the down arrow and confirm a row is selected and marked again.
      A cursor that lands on a neighbour here would put a live agent one unasked keypress from
      being stopped.
...
holding what the Telegram surface writes about itself; in steady state only `serve` opens that
one, so nothing in this section is about it. One exception, during the single upgrade that
creates it: whichever command runs first — `tui` and `pane` included — is what creates and
migrates `ui.sqlite3`, because every one of them splits the surface tables out before opening
the domain store. After that first run, `serve` is its only reader. Which tables are where, and why, is DEC-090. The terminal refuses
