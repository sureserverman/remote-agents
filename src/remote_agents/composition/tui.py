...
def local_context(config, connection, paths: ProductionPaths):
    ...
    if console_hosted:
        ...
        # happen: an absent gate refuses every session key, so the failure mode of a surprise
        # is a layer that does not work rather than one that acts on the wrong console's
        # selection.
        ...
        # The bar's two flags, straight onto the gateway for `console_publish_selection`'s
