...
#: was measured firing it by that name, beside `SessionStart`, in a hook-event log that day; a
#: live drill against a real `codex` is to be added in `tests/live/test_codex_activity_hooks.py`
#: (BL-108's plan, Task 3.2). If a Codex build renamed it, the marker would simply never start and
#: the relay would read the screen alone, as before.
