...
def inspect_capture(
    ...
    *,
    ...
    if b"\x00" in raw:
        return InspectionResult("refused", "binary output cannot be displayed.", None, None, False)
    ...
    if len(text.encode("utf-16-le")) // 2 <= telegram_limit:
        return InspectionResult("text", text, None, None, redacted)
    return InspectionResult(
        "attachment",
        "Output is attached as UTF-8 text.",
        text.encode(),
        "session-output.txt",
        redacted,
    )
