...
ASK_WORDS = {AskClass.SHELL: "about a shell command", AskClass.EDIT: "about editing a file"}
...
def feed_rows(
    ...
    **The fallback is a feature, not a guard.** A notification outlives the session it is
    ...
    for activity, age_text in zip(shown, ages, strict=True):
        ...
        # The class only when the words are absent. This briefly read `_ask_words(activity)`
        # unconditionally (2026-09-19, with DEC-098), on the argument that the class and the
        # command say different things. They do -- but not differently enough to spend a line
        # on both: the owner read the result as redundant, and the row already carries `needs
        # answer` in its own column. `feed_row_content` states the rule.
