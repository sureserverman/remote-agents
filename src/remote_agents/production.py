...
class ProductionPaths:
    ...
    def ensure_directories(self) -> None:
        ...
        for path in (self.config_directory, self.state_directory, self.unit_directory):
            if path.exists() and not path.is_dir():
                raise ConfigError(f"production path is not a directory: {path}")
            path.mkdir(parents=True, exist_ok=True, mode=0o700)
            os.chmod(path, 0o700)
        self.intent_directory.mkdir(exist_ok=True, mode=0o700)
        os.chmod(self.intent_directory, 0o700)
